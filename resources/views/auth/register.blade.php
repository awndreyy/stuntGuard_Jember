...
<body class="min-h-screen bg-[#fdf9f6] relative flex items-center justify-center p-3 sm:p-6 py-6 sm:py-10 overflow-x-hidden overflow-y-auto">
  ...
  <!-- Aksen blur tengah -->
  <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[550px] h-[550px] bg-[#ffd9dd]/20 rounded-full blur-[100px] pointer-events-none"></div>

  <!-- KARTU FORM REGISTRASI -->
  <div class="w-full max-w-[460px] sm:max-w-lg bg-white/95 backdrop-blur-md rounded-2xl shadow-xl shadow-[#a5304d]/10 border border-[#ddbfc2]/50 overflow-hidden relative z-10 my-auto">
    ...
    <div class="p-5 sm:p-8">
      <div class="mb-4 sm:mb-5 text-center">
        <h2 class="text-base sm:text-lg font-bold text-[#1c1b1a]">Registrasi Akun</h2>
        <p class="text-xs text-[#574144] mt-0.5">Lengkapi data diri Anda di bawah ini</p>
      </div>
      ...
      <form action="{{ Route::has('register.store') ? route('register.store') : (Route::has('register') ? route('register') : route('users.store')) }}" method="POST" id="registerForm" class="space-y-3.5 sm:space-y-4">
        ...
        <!-- Input Nama Lengkap -->
        <div>
          <label for="nama" class="block text-xs font-semibold text-[#1c1b1a] mb-1 sm:mb-1.5">
            Nama Lengkap <span class="text-rose-500">*</span>
          </label>
          <div class="relative group">
            <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
              <i data-lucide="user" class="w-4 h-4 {{ ($errors->has('name') || $errors->has('nama')) ? 'text-rose-500' : 'text-[#574144] group-focus-within:text-[#a5304d]' }} transition-colors"></i>
            </div>
            <input
              type="text"
              id="nama"
              ...
              class="input-field w-full pl-10 pr-3.5 py-2.5 text-sm bg-[#f7f3f0] border {{ ($errors->has('name') || $errors->has('nama')) ? 'border-rose-300 ring-1 ring-rose-300 bg-rose-50/30' : 'border-[#ddbfc2] focus:border-[#a5304d]' }} rounded-xl text-[#1c1b1a] placeholder-[#574144]/60 focus:outline-none focus:bg-white transition-all shadow-sm"
            >
          </div>
          @error('name')
        ...
        <div>
          <div class="flex flex-wrap items-center justify-between gap-1 mb-1 sm:mb-1.5">
            <label for="nik" class="block text-xs font-semibold text-[#1c1b1a]">
              Nomor Induk Kependudukan (NIK) <span class="text-rose-500">*</span>
            </label>
            <span id="nikCounter" class="text-[11px] font-medium text-[#574144]/60">0/16 Digit</span>
          </div>
          <div class="relative group">
            <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
              <i data-lucide="credit-card" class="w-4 h-4 {{ $errors->has('nik') ? 'text-rose-500' : 'text-[#574144] group-focus-within:text-[#a5304d]' }} transition-colors"></i>
            </div>
            <input
              type="text"
              id="nik"
              ...
              class="input-field w-full pl-10 pr-3.5 py-2.5 text-sm bg-[#f7f3f0] border {{ $errors->has('nik') ? 'border-rose-300 ring-1 ring-rose-300 bg-rose-50/30' : 'border-[#ddbfc2] focus:border-[#a5304d]' }} rounded-xl text-[#1c1b1a] placeholder-[#574144]/60 focus:outline-none focus:bg-white transition-all shadow-sm tracking-wide"
            >
          </div>
          <p id="nikHelp" class="text-[11px] text-[#574144]/60 mt-1">Harus berupa 16 digit angka sesuai KTP</p>
          @error('nik')
            <p class="text-[11px] text-rose-600 mt-1 flex items-center gap-1">
              <i data-lucide="alert-circle" class="w-3.5 h-3.5"></i>
              <span>{{ $message }}</span>
        ...
        <!-- Input Email -->
        <div>
          <label for="email" class="block text-xs font-semibold text-[#1c1b1a] mb-1 sm:mb-1.5">
        ...
        <!-- Input Password -->
        <div>
          <label for="password" class="block text-xs font-semibold text-[#1c1b1a] mb-1 sm:mb-1.5">
          ...
          <p class="text-[11px] text-[#574144]/60 mt-1">Gunakan minimal 6 karakter</p>
          @error('password')
            <p class="text-[11px] text-rose-600 mt-1 flex items-center gap-1">
              <i data-lucide="alert-circle" class="w-3.5 h-3.5"></i>
              <span>{{ $message }}</span>
        ...
        <div class="flex items-start gap-2.5 pt-1">
          <input
            ...
            name="terms"
            required
            class="mt-0.5 h-4 w-4 rounded border-[#ddbfc2] text-[#a5304d] focus:ring-[#a5304d]/30 accent-[#a5304d] cursor-pointer shrink-0"
          >
          <label for="terms" class="text-xs text-[#574144] select-none cursor-pointer leading-relaxed">
            Saya menyetujui bahwa data yang saya masukkan adalah benar untuk keperluan sistem monitoring <span class="font-medium text-[#a5304d]">StuntGuard</span>.
          </label>
        </div>
        @error('terms')
          <p class="text-[11px] text-rose-600 mt-1 flex items-center gap-1">
        ...
        <div class="pt-2 sm:pt-3">
          <button
            type="submit"
            class="btn-primary w-full flex items-center justify-center gap-2 text-white font-semibold py-2.5 sm:py-3 px-4 rounded-xl shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#a5304d] transition-all cursor-pointer text-sm sm:text-base"
          >
      ...
      <div class="mt-4 sm:mt-5 pt-3 sm:pt-4 border-t border-[#ddbfc2]/30 text-center">
        <p class="text-xs sm:text-sm text-[#574144]">
          Sudah memiliki akun?
          <a href="{{ Route::has('login') ? route('login') : url('/login') }}" class="font-semibold text-[#a5304d] hover:text-[#881938] hover:underline inline-flex items-center gap-1 ml-0.5">
            Masuk di sini
          </a>
        </p>
      </div>
  ...
  <script>
    ...
    function updateNikCounter() {
      ...
      if (length === 16) {
        nikCounter.className = 'text-[11px] font-semibold text-[#236b44]';
        nikHelp.textContent = '✓ Format NIK valid (16 digit)';
        nikHelp.className = 'text-[11px] text-[#236b44] mt-1';
      } else if (length > 0) {
        ...
        nikCounter.className = 'text-[11px] font-medium text-[#574144]/60';
        nikHelp.textContent = 'Harus berupa 16 digit angka sesuai KTP';
        nikHelp.className = 'text-[11px] text-[#574144]/60 mt-1';
      }
    }
