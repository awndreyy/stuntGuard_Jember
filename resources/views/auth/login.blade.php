...
<body class="min-h-screen bg-stone-50 relative flex items-center justify-center p-3 sm:p-6 py-6 sm:py-10 overflow-x-hidden overflow-y-auto">
  ...
  <!-- Aksen blur tengah -->
  <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[480px] h-[480px] bg-rose-100/20 rounded-full blur-[100px] pointer-events-none"></div>

  <!-- KARTU FORM LOGIN -->
  <div class="w-full max-w-[420px] sm:max-w-md bg-white/95 backdrop-blur-md rounded-2xl shadow-xl shadow-rose-700/10 border border-rose-200/50 overflow-hidden relative z-10 my-auto">
    ...
    <div class="p-5 sm:p-8">
      <h2 class="text-base sm:text-lg font-bold text-stone-900 mb-5 sm:mb-6 text-center">Login ke StuntGuard</h2>
      ...
      <form action="{{ Route::has('login') ? route('login') : url('/login') }}" method="POST" class="space-y-4 sm:space-y-5">
        ...
        <div>
          <div class="flex items-center justify-between mb-1.5">
            <label for="password" class="block text-xs font-semibold text-stone-900">Password</label>
            <a href="{{ Route::has('password.request') ? route('password.request') : '#' }}" class="text-xs font-medium text-rose-700 hover:text-rose-900 transition-colors">Lupa Password?</a>
          </div>
          <div class="relative group">
            <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
        ...
        <div class="pt-2">
          <button
            type="submit"
            class="btn-primary w-full flex items-center justify-center gap-2 text-white font-semibold py-2.5 sm:py-3 px-4 rounded-xl shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-700 cursor-pointer text-sm sm:text-base"
          >
      ...
      <div class="mt-5 sm:mt-6 pt-4 sm:pt-5 border-t border-rose-200/30 text-center">
        <p class="text-xs sm:text-sm text-stone-600">
          Belum punya akun?
          <a href="{{ Route::has('register') ? route('register') : url('/register') }}" class="font-semibold text-rose-700 hover:text-rose-900 hover:underline inline-flex items-center gap-1 ml-0.5">
            Daftar di sini
          </a>
        </p>
      </div>
