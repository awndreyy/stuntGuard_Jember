...
<body class="min-h-screen bg-slate-50 relative flex items-center justify-center p-3 sm:p-6 py-6 sm:py-10 overflow-x-hidden overflow-y-auto">
  ...
  <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 sm:w-[500px] h-80 sm:h-[500px] bg-teal-100/40 rounded-full blur-[80px] sm:blur-[100px] pointer-events-none"></div>

  <!-- KARTU FORM LOGIN -->
  <div class="w-full max-w-[420px] sm:max-w-md bg-white/95 backdrop-blur-md rounded-2xl shadow-xl shadow-slate-900/5 border border-slate-200/80 overflow-hidden relative z-10 my-auto">
    ...
    <div class="px-4 sm:px-8 pt-10 sm:pt-8 pb-6 sm:pb-7 text-center border-b border-slate-100 bg-gradient-to-b from-teal-50/70 to-teal-50/30 relative overflow-hidden">
      ...
      <a href="{{ url('/') }}" onclick="if(window.history.length > 1) { history.back(); return false; }" class="absolute top-3.5 left-3.5 sm:top-4 sm:left-4 z-20 inline-flex items-center gap-1.5 text-xs font-semibold text-slate-600 hover:text-teal-900 active:scale-95 bg-white/90 hover:bg-white backdrop-blur-xs px-2.5 py-1.5 rounded-lg border border-slate-200/80 shadow-xs transition-all" title="Kembali ke halaman sebelumnya">
      ...
      <div class="mx-auto w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-teal-800 text-white flex items-center justify-center shadow-md shadow-teal-900/20 mb-3 sm:mb-3.5 relative z-10">
        <i data-lucide="shield-check" class="w-7 h-7 sm:w-8 sm:h-8 stroke-[2.2]"></i>
      </div>
    ...
    <div class="p-5 sm:p-8">
      <h2 class="text-base sm:text-lg font-bold text-slate-800 mb-5 sm:mb-6 text-center">Login ke StuntGuard</h2>
      ...
      <form action="{{ Route::has('login') ? route('login') : url('/login') }}" method="POST" class="space-y-4 sm:space-y-5">
        ...
        <div>
          <div class="flex items-center justify-between mb-1.5">
            ...
            <a href="{{ Route::has('password.request') ? route('password.request') : '#' }}" class="text-xs font-medium text-teal-700 hover:text-teal-900 transition-colors">Lupa Password?</a>
          </div>
          <div class="relative group">
            <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
      ...
      <div class="mt-5 sm:mt-6 pt-4 sm:pt-5 border-t border-slate-100 text-center">
        <p class="text-xs sm:text-sm text-slate-600">
          Belum punya akun? 
          <a href="{{ Route::has('register') ? route('register') : url('/register') }}" class="font-semibold text-teal-800 hover:text-teal-950 hover:underline inline-flex items-center gap-1 ml-0.5">
            Daftar di sini
          </a>
        </p>
      </div>
